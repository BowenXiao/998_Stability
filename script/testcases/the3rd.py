...
class The3rdTest(unittest.TestCase):
	...
	def testNavigation(self):
		...
		assert d(packageName = 'com.autonavi.minimap').wait.exists(timeout = 5000),'Launch Navigation failed in 5s!'
		# turn off notification
		if d(text = '系统提示').exists:
			d(text = '不再显示提醒!').click.wait()
			d(text = '同意').click.wait()
		if d(text = '欢迎使用高德地图!').exists:
			d(text = '同意').click.wait()
		for i in range(10):
		...
		d(textContains = '我的位置').wait.exists(timeout = 30000)
		assert d(textContains = '我的位置').exists,'Location failed, My position is not show on the screen!'
		d.press('back')
		d.sleep(1)
		d.press('back')
		d.press('back')
	...
	def testGame(self):
		...
		# swipe up
		for i in range(10):
			d.swipe(540,1030,540,540,10)
			d.sleep(2)
		# swipe down
		for i in range(10):
			d.swipe(540,540,540,1030,10)
			d.sleep(2)
		# swipe right
		for i in range(10):
			d.swipe(300,960,800,960,10)
			d.sleep(2)
	...
	def testYouku(self):
		...
		if d(textContains = '优酷Youku软件最终用户').exists:
			d(text = '同意').click.wait()
		if d(resourceId = 'com.youku.phone:id/game_image').exists:
			d(text = '取消').click.wait()
