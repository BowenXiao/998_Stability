...
class TelephonyTest(unittest.TestCase):
	...
	def testCallDialerViaSim2(self):
		...
		d(resourceId = 'com.android.contacts:id/switch_sim').long_click()
	...
	def testAddDelContact(self):
		...
		while d(text = 'adele').exists:
			d(text = 'adele').click.wait()
			d(text = '删除此联系人').click.wait()
			d(text = '确认删除').click.wait()

		#Add new contact
		d(text = '添加').click.wait()
		if d(text = '本地保存').wait.exists(timeout = 5000):
			d(text = '本地保存').click.wait()
		assert d(resourceId = 'com.android.contacts:id/frame').wait.exists(timeout = 5000),'Switch to create contacts failed in 5s!'
	...
	def _setMobileNetWork(self,net):
		...
		if not d(text='蜂窝移动数据').wait.exists(timeout=1000):
			for i in range(3):
				d.swipe(540, 700, 540, 1400, 10)
				d.sleep(1)
	...
	def _operateDuringCalling(self):
		...
		if not d(resourceId = 'com.android.incallui:id/elapsedTime').wait.exists(timeout = 30000):
			d(text = '结束').click.wait()
		assert d(resourceId = 'com.android.incallui:id/elapsedTime').wait.exists(timeout = 30000), 'Calling failed to connect in 30s'
		#Random click a number
		self._makeNumber()
